...
const searchUrl = 'https://jkanime.net/buscar/';
...
const getAnimeVideo = async (id: string, chapter: number) => {
  const res = await fetch(`${url}${id}/${chapter}/` , {headers: {"Access-Control-Allow-Origin": "*"}});
  const body = await res.text();
  //const options = {
  //  resources: 'usable',
...
export { searchAnime, getAnimeVideo }